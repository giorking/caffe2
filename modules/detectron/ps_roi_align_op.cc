...
REGISTER_CPU_OPERATOR(PSRoIAlign, PSRoIAlignOp<float, CPUContext>);
REGISTER_CPU_OPERATOR(
    PSRoIAlignGradient,
    PSRoIAlignGradientOp<float, CPUContext>);

OPERATOR_SCHEMA(PSRoIAlign)
    .NumInputs(2)
    .NumOutputs(2)
    .SetDoc(R"DOC(
Position Sensitive Region of Interest Aligning as used in R-FCN.
)DOC")
    .Arg(
        "spatial_scale",
...
OPERATOR_SCHEMA(PSRoIAlignGradient)
    .NumInputs(4)
    .NumOutputs(1)
    .Input(
        0,
...
class GetPSRoIAlignGradient : public GradientMakerBase {
...
REGISTER_GRADIENT(PSRoIAlign, GetPSRoIAlignGradient);
